...
import { Category, Product, PaginatedResponse, ApiResponse } from "@/lib/types";
...
export default function CategoryPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const categoriesData = (catRes.data || catRes) as PaginatedResponse<Category>;
        
        if (!categoriesData || !categoriesData.items) {
          throw new Error("Invalid categories response structure");
        }

        const found = categoriesData.items.find((c: Category) => c.slug === slug);
        ...
        setError("Có lỗi xảy ra khi tải dữ liệu. Vui lòng kiểm tra kết nối hoặc đăng nhập lại.");
      } finally {
        setLoading(false);
      }
    }
  ...
  if (error || !category) return (
    <div className="max-w-7xl mx-auto p-20 text-center">
        <h1 className="text-3xl font-black text-gray-900">{error || "Không tìm thấy danh mục"}</h1>
        <p className="text-gray-500 mt-4">Vui lòng quay lại <Link href="/" className="text-primary hover:underline font-bold">Trang chủ</Link>.</p>
    </div>
  );
      ...
      <div className="flex flex-col md:flex-row gap-10">
        ...
        <aside className="w-full md:w-64 space-y-8">
            <div className="space-y-4">
                <h3 className="font-bold text-gray-900 uppercase text-xs tracking-widest">Sort By</h3>
                <div className="space-y-2">
                    {["Featured", "Price: Low to High", "Price: High to Low"].map(opt => (
                        <label key={opt} className="flex items-center gap-3 cursor-pointer group">
                             ...
                             <span className="text-sm font-medium text-gray-600 group-hover:text-gray-900">{opt}</span>
                        </label>
                    ))}
                </div>
            </div>
        ...
        <main className="flex-1">
            <div className="flex justify-between items-center mb-6">
                <p className="text-sm text-gray-500 font-medium">Showing {products.length} results</p>
            </div>
